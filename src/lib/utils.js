...
export function processFormData(formData) {
  ...
  return Array.from(formData.entries()).reduce((result, [key, value]) => {
    // Правим баг, который мешает фильтрации по полю Total
    if (key == 'total[]' && !result.total)
      result.total = formData.getAll('total[]');

    if (key != 'total[]') result[key] = value;
    // END Правим баг, который мешает фильтрации по полю Total
